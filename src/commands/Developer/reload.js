...
module.exports = class Reload extends Command {
  ...
  async run (msg, args, flags) {
    ...
    let eventResult, commandsResult, commandResult, commandName, res

    switch (args[0] ? args[0].toLowerCase() : null) {
      case 'events':
        msg.client.logger.warn('Force event reload')

        eventResult = await msg.client.loadEvents
      ...
      case 'commands':
        ...
        commandsResult = await msg.client.cmd.load()
      ...
      case 'command':
        ...
        commandResult = await msg.client.cmd.reloadCommand(commandName)

        if (commandResult.status === 500) { m.edit(msg.error(commandResult.message)) } else if (commandResult.status === 200) { m.edit(msg.success(commandResult.message)) } else if (commandResult.status === 404) { m.edit(msg.warning(commandResult.message)) }

        break

      default:
        res = await msg.client.reload()
