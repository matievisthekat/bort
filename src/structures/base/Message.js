...
const { xpForLevel } = require('../util/util')
...
class MsgExtension extends Message {
  ...
  async run () {
    ...
    if (!this.guild) {
      ...
      if (this.content.toLowerCase().startsWith(prefix)) {
        if (this.cooldown.has(this.guild ? this.guild.id : this.author.id)) { return }
  ...
  async prefix (includeMention) {
    ...
    if (this.content.match(prefixMention) && includeMention) { prefix = this.content.match(prefixMention)[0] } else if (data !== null) prefix = data.prefix
  ...
  async translate (text) {
    ...
    if (!this.author.langModel) { await this.author.loadLang(userLang ? userLang.lang : this.author.lang) }
  ...
  async commandCheck (cmd, args, flags) {
    ...
    if (cmd.config.guildOnly && !this.guild) { return this.client.errors.guildOnly(this, this.channel) }
    ...
    if (cmd.config.voiceChannelOnly) {
      if (!this.member.voice || !this.member.voice.channel) { return this.client.errors.voiceChannelOnly(this, this.channel) }
