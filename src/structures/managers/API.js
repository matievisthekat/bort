  ...
  constructor (client, port) {
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        const data =
          (await this.client.models.Prefix.findOne({
            guildID
          })) ||
          new this.client.models.Prefix({
            guildID
          })
    ...
    app.get(`/api/${client.config.api.version}/commands/search`, (req, res) => {
      ...
      if (query.split(':')[0] && query.split(':')[1]) {
        results = client.cmd.commands.array().filter((cmd) => {
          ...
          } else if (Array.isArray(param)) { result = param.includes(query.toLowerCase().split(':')[1]) }
    ...
    app.get(`/api/${client.config.api.version}/events`, (req, res) => {
      const events = this.client.evnt.events.array()
      res.send({ events }).status(200)
    })

    // POST request to execute a command on the server
    app.post(`/api/${client.config.api.version}/exec`, async (req, res) => {
      if (req.body.AUTH !== client.config.api.AUTH) { return res.send({ success: false, message: 'Unauthorized. Go away' }) }
